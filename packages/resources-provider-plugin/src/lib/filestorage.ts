    ...
    try {
      ...
      for (const f in files) {
        ...
        try {
          ...
          // is bad and skip it rather than failing the whole listing. A
          // skipped file does not consume a `limit` slot. Only read/parse
          // failures are caught here so genuine filter/metadata bugs still
